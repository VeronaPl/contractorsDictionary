 ...
 * @returns {{ name: string, type: string, inn: string, number: string, dataTime: string, telephone: string, email: string }} Объект клиента
 */
function makeClientFromData(data) {
  ...
  const client = {
    ...
    type:
      typeof data.type === "string" || data.type instanceof String
        ...
        : "",
    inn:
      typeof data.inn === "string" || data.inn instanceof String
        ? asString(data.inn)
        : "",
    number:
      typeof data.number === "string" || data.number instanceof String
        ? asString(data.number)
        : "",
    dataTime:
      typeof data.dataTime === "string" || data.dataTime instanceof String
        ? asString(data.dataTime)
        : "",
    telephone:
      typeof data.telephone === "string" || data.telephone instanceof String
        ? asString(data.telephone)
        : "",
    email:
  ...
  if (!client.name)
    errors.push({ field: "name", message: "Не указано название" });
 ...
 * @returns {{ id: string, name: string, type: string, inn: string, number: string, dataTime: string, telephone: string, email: string }[]} Массив клиентов
 */
 ...
 * @returns {{ id: string, name: string, type: string, inn: string, number: string, dataTime: string, telephone: string, email: string, createdAt: string, updatedAt: string }} Объект клиента
 */
 ...
 * @param {{ name?: string, type?: string, inn?: string, number?: string, dataTime?: string, telephone?: string, email?: string }} data - Объект с изменяемыми данными
 * @throws {ApiError} Клиент с таким ID не найден (statusCode 404)
 * @throws {ApiError} Некорректные данные в аргументе (statusCode 422)
 * @returns {{ id: string, name: string, type: string, inn: string, number: string, dataTime: string, telephone: string, email: string, createdAt: string, updatedAt: string }} Объект клиента
 */
...
module.exports = createServer(async (req, res) => {
  ...
  .on("listening", () => {
    if (process.env.NODE_ENV !== "test") {
      ...
      console.log(
        `POST ${URI_PREFIX} - создать клиента, в теле запроса нужно передать объект { name: string, type?: string, inn?: string, number?: string, dataTime?: string, telephone?: string, email?: string }`
      );
      console.log(`GET ${URI_PREFIX}/{id} - получить клиента по его ID`);
      console.log(
        `PATCH ${URI_PREFIX}/{id} - изменить клиента с ID, в теле запроса нужно передать объект { name?: string, type?: string, inn?: string, number?: string, dataTime?: string, telephone?: string, email?: string }`
      );
